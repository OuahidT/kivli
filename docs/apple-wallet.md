# Apple Wallet — infrastructure Kivli
...
Aucune clé privée, aucun certificat et aucun jeton ne doivent être ajoutés à GitHub. Le certificat et la clé servent aussi à la liaison mTLS Cloudflare `APPLE_WALLET_APNS`, utilisée uniquement pour notifier Wallet qu’un pass installé doit être rechargé.
...
4. vérifier la signature PKCS#7 détachée, le manifeste SHA-1 et les images du paquet, puis générer le premier `.pkpass` ;
