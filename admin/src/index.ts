...
<body>
  ...
  <script nonce="${nonce}">
    ...
    async function deleteMerchant(){if(!state.selected)return;const merchant={id:state.selected.id,businessName:state.selected.businessName};const confirmation=prompt('Suppression définitive : saisis exactement "'+merchant.businessName+'" pour confirmer.');if(confirmation===null)return;if(confirmation.trim()!==merchant.businessName){toast("Le nom saisi ne correspond pas. Le compte n’a pas été supprimé.",true);return}const button=$("delete-merchant");button.disabled=true;button.textContent="Suppression…";try{await api("/api/merchants/"+encodeURIComponent(merchant.id),{method:"DELETE",body:JSON.stringify({confirmation:confirmation.trim()})});closePanel();toast("Compte "+merchant.businessName+" supprimé définitivement.");await refreshAll()}catch(error){button.disabled=false;button.textContent="Supprimer définitivement";toast(error.message||"Suppression impossible.",true)}}
