      ...
      <section className="dashboard-main">
        ...
        {tab === "scan" && (
          <div className="scan-layout">
            <div>{data.merchant.role === "owner" && <span className="eyebrow scan-eyebrow"><ScanLine size={15} aria-hidden="true" />Ajout instantané</span>}<h2>{data.merchant.role === "employee" ? "Présente le QR du client." : "Scanne la carte du client."}</h2><p>{data.merchant.role === "employee" ? "Choisis le nombre de points, puis ouvre la caméra." : "Choisis le nombre d’achats, puis scanne le QR. Une seconde validation rapprochée reste possible après confirmation."}</p><MerchantScanner onDetected={stamp} busy={busy} /></div>
