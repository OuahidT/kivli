    ...
    <section className={`program-onboarding-shell ${showForm ? "form-open" : ""}`}>
      ...
      <div className="program-setup-stage">
        {showForm ? <form className="program-setup-form" onSubmit={createProgram}>
          ...
          <section className="setup-section identity-section"><div className="setup-section-heading"><span>01</span><div><h3>L’identité de votre carte</h3><p>Le nom que vos clients verront sur leur carte digitale.</p></div></div><div className="setup-fields"><label>Nom du commerce<input value={data.merchant.businessName} disabled /></label><label>Nom de la carte<input name="name" defaultValue="Ma carte fidélité" maxLength={80} required /></label></div></section>
          <section className="setup-section"><div className="setup-section-heading"><span>02</span><div><h3>Comment gagner des points ?</h3><p>Choisissez la règle la plus naturelle pour votre activité.</p></div></div><fieldset className="earning-mode premium-mode-picker"><legend className="sr-only">Mode de fidélité</legend><label className={earningMode === "visits" ? "selected" : ""}><input type="radio" name="earningMode" value="visits" checked={earningMode === "visits"} onChange={() => setEarningMode("visits")} /><span className="mode-icon"><Footprints size={22} aria-hidden="true" /></span><span><strong>Par passage</strong><small>1 passage ou achat = 1 point</small></span><Check className="mode-check" size={17} aria-hidden="true" /></label><label className={earningMode === "spend" ? "selected" : ""}><input type="radio" name="earningMode" value="spend" checked={earningMode === "spend"} onChange={() => setEarningMode("spend")} /><span className="mode-icon"><Coins size={22} aria-hidden="true" /></span><span><strong>Selon le montant dépensé</strong><small>Les points suivent le montant de l’achat</small></span><Check className="mode-check" size={17} aria-hidden="true" /></label></fieldset>{earningMode === "spend" && <div className="spend-rule-card"><span className="spend-rule-icon"><Coins size={20} aria-hidden="true" /></span><div><small>RÈGLE APPLIQUÉE</small><strong>1 point par euro dépensé</strong><p>Un achat de 12 € ajoutera automatiquement 12 points.</p></div></div>}<input type="hidden" name="spendAmountEuros" value="1" /></section>
          <section className="setup-section"><div className="setup-section-heading"><span>03</span><div><h3>Vos récompenses</h3><p>Définissez le premier objectif, puis ajoutez un second palier si nécessaire.</p></div></div><fieldset className="reward-tiers-editor premium-rewards"><legend className="sr-only">Paliers de récompense</legend><div className="reward-tier-row"><span className="tier-number">1</span><label>Points nécessaires<input name="tierThreshold" type="number" min="1" max="1000" defaultValue="8" required /></label><label>Récompense offerte<input name="tierRewardText" placeholder="Ex. Un café offert" maxLength={120} required /></label></div><div className="reward-tier-row optional"><span className="tier-number">2</span><label>Points nécessaires <small>Facultatif</small><input name="tierThreshold" type="number" min="1" max="1000" /></label><label>Deuxième récompense <small>Facultatif</small><input name="tierRewardText" placeholder="Ex. 20 % de réduction" maxLength={120} /></label></div></fieldset><input type="hidden" name="rewardText" value="Récompense fidélité" /></section>
          <section className="setup-section"><div className="setup-section-heading"><span>04</span><div><h3>La couleur de votre carte</h3><p>Choisissez la teinte qui représente le mieux votre commerce.</p></div></div><fieldset className="color-fieldset program-colors premium-colors"><legend className="sr-only">Couleur de la carte</legend><div className="color-options">{PROGRAM_COLORS.map((color, index) => <label key={color.value} className="color-choice" style={{ backgroundColor: color.value }} title={color.name}><input type="radio" name="accentColor" value={color.value} defaultChecked={index === 0} aria-label={color.name} /><span>{color.name}</span></label>)}</div></fieldset></section>
          <section className="setup-section optional-section"><div className="setup-section-heading"><span>05</span><div><h3>Conditions du programme</h3><p>Facultatif · Ajoutez uniquement une règle utile à vos clients.</p></div></div><label className="standalone-field">Conditions affichées au client<textarea name="terms" rows={3} maxLength={200} placeholder="Ex. Offre non cumulable avec une promotion en cours." /></label></section>
